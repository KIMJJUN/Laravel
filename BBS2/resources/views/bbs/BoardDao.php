...
class BoardDAO {
	...
	public function insertMsg($title, $writer, $content) {
		...
		// 실행요청하고..
		try {
			$sql = "insert into board(title, writer, content,regtime,hits) values(:title, :writer, :content,:regtime,0)";
	...
	public function updateMsg($num, $title, $writer, $content) {
		try {
			$sql = "update board set title=:t, writer=:w, content=:c, regtime=:r where num=:n"; 
			/*
				1. prepare
			...
			*/
             ...
             $pstmt->bindValue(":r", $regtime, PDO::PARAM_STR);
